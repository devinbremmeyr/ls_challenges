...
create a signly linked list
can contain a range of data, such as the numbers 1-10
Provide functions to reverse the linked list

provide function to convert linked list to an from an array

Examples test cases:

class Element
  constructed with a piece of data
  and a pointer to the next element in the list

class SimpleLinkedList
  constructed as an empty list

  ::from_a => transform an Array into a new list object

  reverse  => return a new list linked in reverse

  size     => a integer representing a count of the elements in list

  empty?   => true if list contains no elements

  peek     => return "next" element w/o advancing list position

  head     => return the first element 

  push(d)  => Link head to element.new(d, head)

  pop      => remove the element linked to the last element of list
              returning datum

  to_a     => collect all elements in an array
