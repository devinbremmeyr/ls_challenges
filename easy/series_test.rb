...
take a STRING of digits and return all the possible consecutive number
...
asked for a series of size greater than given string: throw an error

Examples test cases:

class Series
  construcutor takes string of digits

  #slices(series length)
    => array of, arrays of integers with size series_length
    => order of numbers matches from original string of digits
    => raises error if series_length > digits
...
Parse through each index of string
digts = '01234'
digits.chars.map(&:to_i).each_cons(2) { |a| p a }
=end
